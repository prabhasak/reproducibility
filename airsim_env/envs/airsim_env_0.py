...
Description:
	...
	A drone (with yaw = 0) flies from a random takeoff point to its landing spot
	The takeoff and landing operations are performed using airsim APIs
	Some env_kwargs() are passed according to user requirements
...
class AirSim(gym.Env):
	...
	def __init__(self, name='', rew_complexity='simple', rew_land=1000, restrict=False):

		# NOTE (VERY IMPORTANT): These values are for the RL-generated expert data titled 'AirSim-v0_1.npz' and 'AirSim-v0_2.npz'
		# These can also be passed as env_kwargs()
		self.name = name
		self.restrict = restrict
		self.reward_landing = rew_land
		self.reward_complexity = rew_complexity
		...
		self.name_folder = 'C:/IL_UAV/GAIL-TF-SB/data/airsim/logs' # Laptop - new
		# self.name_folder = 'G:/UAV/GAIL-TF-SB/data/logs' # Laptop - old
	...
	def reset(self):
		if not self.client.isApiControlEnabled(vehicle_name=self.name):
		...
		self.client.moveToPositionAsync(ux, uy, uz, 1, vehicle_name=self.name).join()
		self.client.rotateToYawAsync(yaw=0, vehicle_name=self.name).join()
	...
	def reward_function(self):
		...
		# if self.x > self.observation_ub[0]: #Check how often is drone going out of bounds
		# 	print('Out of bounds!')
	...
	def step(self, action):
		...
		# action_org = action
		# action_org = action*(self.action_ub-self.action_lb)+self.action_lb
		# action_org = np.clip(action, self.action_lb, self.action_ub) # 1- un-normalize through clip, if data not normalized
		...
		self.client.moveByAngleThrottleAsync(float(self.pitch), float(self.roll), float(self.throttle), float(0), duration=self.time_step, vehicle_name=self.name).join() #default vehicle_name=''
		...
		# 	# print('actions_unclipped: ', action)
		# 	print('actions: ', action_org)
		# print('reward: ', reward)
